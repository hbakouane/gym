<!-- testimonial-area start -->
<div class="testimonial-area bg-white pt-7 pb-20" id="testimonials">
    <div class="container">
        <h2 class="text-center font-size-20 text-default-color-2 letter-spacing-np3 font-weight-bold font-family-5 mb-6">{{ __('external.What are our clients saying about us?') }}</h2>
        <div class="row no-gutters border-collapse-1">
            <div class="col-lg-4 col-md-6 col-sm-9 pr-lg-0">
                <div class="testimonial-card text-center pt-10 pb-12 border px-xl-13 px-9">
                    <img src="external/testimonials/1.jpg" class="img-fluid rounded-50p mb-3" style="height: 150px;">
                    <img src="image/l8/png/quote.png" alt="" class="mb-12">
                    <p class="font-size-7 letter-spacing-np4 line-height-1p7 text-default-color-3 mb-0 pr-lg-7 pr-sm-9 h-190 font-family-inter">
                        “{{ __('external.testimonials1') }}”
                    </p>
                    <!-- media start -->
                    <div class="media ml-1 align-items-center">
                        <!-- customer-img start -->
                        <div class="customer-img mr-4">
                            <img src="image/l8/png/client-img-1.png" alt="" class="circle-size-43 w-100">
                        </div>
                        <!-- customer-img end -->
                        <!-- media-body start -->
                        <div class="media-body pl-4 pt-md-0 pt-9">
                            <h5 class="font-size-5 font-family-1 line-height-1p86 font-weight-bold text-default-color-2 mb-0">{{ __('external.testimonials1_client') }}</h5>
                            <p class="font-size-4 letter-spacing-np64 line-height-1p86 font-weight-normal text-dovegray mb-0">{{ __('external.testimonials1_job') }}</p>
                        </div>
                        <!-- media-body end -->
                    </div>
                    <!-- media end -->
                </div>
            </div>
            <div class="col-lg-4 col-md-6 col-sm-9 px-lg-0">
                <div class="testimonial-card text-center pt-10 pb-12 border px-xl-13 px-9">
                    <img src="external/testimonials/2.jpg" class="img-fluid rounded-50p mb-3" style="height: 150px;">
                    <img src="image/l8/png/quote.png" alt="" class="mb-12">
                    <p class="font-size-7 letter-spacing-np4 line-height-1p7 text-default-color-3 mb-0 pr-lg-7 pr-sm-9 h-190 font-family-inter">
                        “{{ __('external.testimonials2') }}”
                    </p>
                    <!-- media start -->
                    <div class="media ml-1 align-items-center">
                        <!-- customer-img start -->
                        <div class="customer-img mr-4">
                            <img src="image/l8/png/client-img-2.png" alt="" class="circle-size-43 w-100">
                        </div>
                        <!-- customer-img end -->
                        <!-- media-body start -->
                        <div class="media-body pl-4">
                            <h5 class="font-size-5 font-family-1 line-height-1p86 font-weight-bold text-default-color-2 mb-0">{{ __('external.testimonials2_client') }}</h5>
                            <p class="font-size-4 letter-spacing-np64 line-height-1p86 font-weight-normal text-dovegray mb-0">{{ __('external.testimonials2_job') }}</p>
                        </div>
                        <!-- media-body end -->
                    </div>
                    <!-- media end -->
                </div>
            </div>
            <div class="col-lg-4 col-md-6 col-sm-9 pl-lg-0">
                <div class="testimonial-card text-center pt-10 pb-12 border px-xl-13 px-9">
                    <img src="external/testimonials/3.jpeg" class="img-fluid rounded-50p mb-3" style="height: 150px;">
                    <img src="image/l8/png/quote.png" alt="" class="mb-12">
                    <p class="font-size-7 letter-spacing-np4 line-height-1p7 text-default-color-3 mb-0 pr-lg-7 pr-md-9 pr-sm-11 h-190 font-family-inter">
                        “{{ __('external.testimonials3') }}”
                    </p>
                    <!-- media start -->
                    <div class="media ml-1 align-items-center">
                        <!-- customer-img start -->
                        <!-- customer-img end -->
                        <!-- media-body start -->
                        <div class="media-body pl-4">
                            <h5 class="font-size-5 font-family-1 line-height-1p86 font-weight-bold text-default-color-2 mb-0">{{ __('external.testimonials3_client') }}</h5>
                            <p class="font-size-4 letter-spacing-np64 line-height-1p86 font-weight-normal text-dovegray mb-0">{{ __('external.testimonials3_job') }}</p>
                        </div>
                        <!-- media-body end -->
                    </div>
                    <!-- media end -->
                </div>
            </div>
        </div>
    </div>
</div>
<!-- testimonial-area end -->
